	...
	fire(coordinate) {
		...
		if (this.enemyMap[coordinate] !== -1) {
			...
			if (ship.size === 0) {
				...
				this.fleetSize--;
				if (this.fleetSize === 0) {
					fireText.innerText = "Game over! You destroyed the fleet in " + this.totalCount + " moves";
	...
	buttonClick(button) {
		...
		if (returnVal >= 0) {
			if (button.style.backgroundColor !== 'red') {
				...
				this.missCount = 0;
				if (this.hitCount > 1) {
					fireText.innerText = ships[returnVal].name + " was hit x" + this.hitCount + "!";
				}
			}
		}
		else if (returnVal === -1) {
			if (button.style.backgroundColor !== 'green' && button.style.backgroundColor !== 'black') {
